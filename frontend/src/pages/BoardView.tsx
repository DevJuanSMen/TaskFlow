...
export const BoardView: React.FC = () => {
  ...
  const { tasks, loading, error, boardInfo, createTask, moveTask } = useBoardFacade(id!);
    ...
    <div className="flex-1 p-6 lg:p-8 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 flex gap-6 overflow-x-auto pb-4">
        {boardInfo?.columns?.map((column: any) => (
          <div 
            key={column._id} 
            className="min-w-[300px] max-w-[320px] flex flex-col bg-surface/30 p-4 rounded-2xl border border-border/50 transition-colors hover:bg-surface/50"
